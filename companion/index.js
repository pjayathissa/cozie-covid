...
settingsStorage.addEventListener("change", function() {
    console.log(
        "settings storage via addEventListner fired (route 2 - disabled). No code here"
    );
...
if (me.launchReasons.settingsChanged) {
    ...
    console.log(
        "settings changed launch reasons fired (route 3 - disabled), code temporarily disabled"
    );
}
...
function sendSettingData(data) {
    ...
    if (messaging.peerSocket.readyState === messaging.peerSocket.OPEN) {
        ...
        console.log(
            "No peerSocket connection. Attempting to send via file transfer"
        );
        ...
        outbox
            ...
            .catch((error) => {
                console.log(
                    `Failed to queue settings for ${evt.key}. Error: ${error}`
                );
            });
    }
}
...
function sendDataToInflux(data) {
    ...
    try {
        user_id = JSON.parse(settingsStorage.getItem("user_id")).name;
    } catch (error) {
    ...
    try {
        experiment_id = JSON.parse(settingsStorage.getItem("experiment_id"))
            .name;
    } catch (error) {
        if (error instanceof TypeError) {
    ...
    fetch(url, {
        ...
        .then((res) => {
            ...
            if (res.status !== 200) {
                sendSettingData({
                    key: "error",
                    value: {
                        selected: { type: "fetchErr", message: res.status },
                    },
        ...
        .catch(function(error) {
            ...
            sendSettingData({
                key: "error",
                value: {
                    selected: { type: "fetchErr", message: error.message },
                },
                time: new Date().getTime(),
            });
        });
